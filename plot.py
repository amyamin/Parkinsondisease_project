def plot_knn(model, X, y, resolution=100, ax=None):
    ...
    cat_map = {c: i for i, c in enumerate(y.unique())}
    new_y_hat = np.empty_like(y_hat, dtype=int)
    for k, v in cat_map.items():
        new_y_hat[y_hat==k] = v
    new_y = y.replace(cat_map)

    if ax is None:
        fig, ax = plt.subplots()
    ax.pcolormesh(grid_x1_values, grid_x2_values, new_y_hat, cmap="Pastel2", alpha=1.0, shading="auto")
    X.plot.scatter(0, 1, c=new_y, colormap="Dark2", colorbar=False, alpha=0.8, ax=ax)